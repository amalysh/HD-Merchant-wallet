...
def convert_from_wei(amount):
    """
    Convert a particular wei amount to eth
    :param amount: Amount in wei unit
    :return: converted eth amount
    """
...
def get_transaction_details(transaction_hash, coin_symbol="eth"):
    """
    Get current transaction details of an hash as returned by blockcypher
    :param transaction_hash: eth address
    :return: transaction and balance information
    """
...
def confirm_transaction_date_without_previous_hash(
    ...
    """
    Check a eth transaction date if it occured before a particular time
...
class EthereumBackend:
    ...
    def generate_new_address(self, index, address_type="p2pkh", derivation_path="m/0"):
        """
        Generate new eth address from a hd public master key based on a particlar index
    ...
    def convert_from_fiat(self, amount, currency="USD"):
        ...
        """
        res = self.converter.convert_to_eth(amount, currency)
        return round(res, 8)
    ...
    def _check_balance_confirmations(
        ...
        sent_eth_amount = convert_from_wei(sent_value)
        if sent_eth_amount < total_crypto_amount:
            remaining_crypto_amount = total_crypto_amount - sent_eth_amount
            return self.UNDERPAID_ADDRESS_BALANCE, remaining_crypto_amount
        return self.CONFIRMED_ADDRESS_BALANCE, sent_eth_amount
